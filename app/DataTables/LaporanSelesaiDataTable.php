...
class LaporanSelesaiDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ->addColumn('laporan_content', function ($row) use ($userTimezone) {
                // Use is_anonymous flag to determine display name
                $pengirim = $row->is_anonymous 
                    ? '<em>Anonim</em>' 
                    : ($row->user ? e($row->user->name) : '<em>Anonim</em>');
                ...
                return '
                    <div class="laporan-card card-selectable p-3 border rounded shadow-sm mb-2"
                        data-id="'.$row->id.'"
                        style="cursor:pointer; line-height:1.4; position: relative;">
    ...
    public function query(Report $model): QueryBuilder
    {
        return $model->newQuery()
            ...
            ->with('user:id,name')
            ->select(['id', 'topic', 'date', 'place', 'chronology', 'status', 'created_at', 'updated_at', 'sender_id', 'is_anonymous']); // Add is_anonymous
    }
