    ...
    @elseif(isset($session) && $session->id_session)
        @if($session->is_active)
            ...
            globalChatHelper.initializeChat({
                ...
                sendUrl: '/serenity/konseling/send',
                fetchUrl: '/serenity/konseling/fetch/{{ $session->id_session }}',
                csrfToken: $('meta[name="csrf-token"]').attr('content'),
                markAsReadUrl: '/serenity/konseling/mark-read/{{ $session->id_session }}'
            });
        @else
            ...
            success: function(response) {
                ...
                if (response.success && response.session_id) {
                    ...
                    if (response.is_new_session) {
                        ...
                        window.history.replaceState({}, '', '/serenity/konseling/session/' + response.session_id);
...
function initializeRealTimeChat(sessionId, chatHelperInstance) {
    ...
    chatHelperInstance.markAsReadUrl = '/serenity/konseling/mark-read/' + sessionId;
...
function loadEndedSessionMessages(sessionId) {
    $.ajax({
        url: '/serenity/konseling/fetch/' + sessionId,
        method: 'GET',
        headers: {
...
function createNewSession() {
    ...
    $.ajax({
        url: '/serenity/konseling/new-session',
        method: 'POST',
        data: {
            _token: $('meta[name="csrf-token"]').attr('content'),
            counselor_id: counselorId,
...
function deleteSession(sessionId) {
    ...
    showConfirm(confirmMessage, function () {
        ...
        $.ajax({
            url: `/serenity/konseling/delete/${sessionId}`,
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                'Accept': 'application/json'
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    setTimeout(() => {
                        window.location.href = '/serenity/konseling';
                    }, 1500);
                } else {
...
// FIXED: Send message to existing session (after first message)
// function sendMessageToExistingSession(sessionId, message, messageInput) {
//     $('#send-btn').prop('disabled', true);
//     $('#send-icon').hide();
//     $('#send-loading').show();
...
//     console.log('📤 Sending message to existing session:', sessionId);
...
//         },
//         success: function(response) {
//             console.log('✅ Message sent to existing session:', response);
...
//             // Use ChatHelper to append (it will use Pusher for broadcasting)
//             const chatHelper = new ChatHelper();
//             chatHelper.appendMessage(messageToAppend, 'student');
