...
$(document).ready(function() {
    ...
    chatHelper.initializeChat({
        ...
        sendUrl: '/admin/konseling/send',
        fetchUrl: `/admin/konseling/fetch/{{ $session->id_session }}`,
        csrfToken: $('meta[name="csrf-token"]').attr('content'),
        markAsReadUrl: '/admin/konseling/mark-read/{{ $session->id_session }}'
    });
...
function loadEndedSessionMessages() {
    ...
    $.ajax({
        url: `/admin/konseling/fetch/{{ $session->id_session }}`,
        method: 'GET',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
            'Accept': 'application/json'
...
function endSession() {
    showConfirm('Are you sure you want to end this session?', function () {
        $.ajax({
            url: `/admin/konseling/end/{{ isset($session) ? $session->id_session : '0' }}`,
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                'Accept': 'application/json'
...
function archiveSession(sessionId) {
    showConfirm('Archive this session? It will be moved to your archive and removed from your active list.', function () {
        $.ajax({
            url: `/admin/konseling/archive/${sessionId}`,
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                'Accept': 'application/json'
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    window.location.href = '/admin/konseling';
                } else {
...
function deleteSession(sessionId) {
    ...
    showConfirm(confirmMessage, function () {
        ...
        $.ajax({
            url: `/admin/konseling/delete/${sessionId}`,
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                'Accept': 'application/json'
