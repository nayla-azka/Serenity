...
.admin-navbar .dropdown-menu {
  ...
  min-width: 120px;
  padding: 4px 0;
  max-width: 90vw;        /* biar tidak overflow ke kanan */
  overflow-x: hidden;     /* cegah horizontal scroll */
  white-space: normal;    /* teks panjang otomatis wrap */
...
#admin-notification-dropdown-menu {
  width: 380px;
  max-width: 90vw;       /* adaptif biar aman di layar kecil */
  right: 0;              /* nempel ke kanan */
  left: auto;            /* cegah geser keluar */
  overflow-x: hidden;
  /*white-space: normal; */
}
...
#admin-notification-button i {
  font-size: 12px; /* default fas fa-bell itu sekitar 1rem–1.25rem */
  line-height: 1;
}

/* Badge lebih kecil */
#admin-notification-badge {
  font-size: 0.55rem;   /* angka di dalam lebih kecil */
  padding: 2px 4px;     /* biar nggak terlalu besar */
  transform: translate(-30%, -30%); /* geser posisinya supaya pas */
}
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function fetchNotifications() {
        ...
        .catch(error => {
            ...
            notifications = []; // Reset to empty array on error
            updateBadge();
    ...
    function createNotificationElement(notification) {
        ...
        // Add click handler to the clickable area only
    ...
    // FIXED: Remove undefined targetName variable
    ...
    window.deleteAllRead = function() {
        ...
        showConfirm('Yakin ingin menghapus notifikasi yang sudah dibaca?', function () {
            ...
            .then(data => {
                ...
                if (data.status === 'success' || data.success) {
                    ...
                    updateBadge();
                    renderNotifications();
                    showToast(`${data.deleted || 0} notifikasi telah dihapus.`);
                } else {
                    showToast('Gagal menghapus notifikasi.');
                }
            })
    ...
    // FIXED: Better redirectToNotification function with proper route matching
    window.redirectToNotification = function(url, notificationId) {
        ...
        // FIXED: Use correct route for marking as read
